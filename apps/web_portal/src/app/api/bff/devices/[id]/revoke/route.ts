...
export async function POST(
  ...
  try {
    ...
    await new GatewayClient(session).revokeDevice((await params).id);
    return NextResponse.redirect(new URL("/devices?result=revoked", portalEnv().PORTAL_PUBLIC_ORIGIN), 303);
  } catch (error) {
