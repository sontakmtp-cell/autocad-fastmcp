...
5. Capture DB no-effect mới; chạy `restart-query` qua public scene path: build scene, stop Gateway process thật, chứng minh PID cũ đã exit, start process mới, Agent reconnect, query scene cũ public.
