...
def main():
    ...
    for skill_id, domain, scope, pure_kind in SKILLS:
        ...
        if skill_id == "drawing.cleanup-audit": steps=[step("query","query",[],{"page_size":run_input("page_size")}),step("pure",pure_kind,["query"],{"entities":ref("query")}),step("report","emit_report",["pure"],{"audit":ref("pure")}),step("review","wait_user_input",["report"],{"report":ref("report")}),step("finish","finish",["review"])]
        elif skill_id.startswith("mechanical.auto"):
            steps=[step("observe","observe",[],{"entity_ids":run_input("entity_ids")}),step("query","query",["observe"],{"snapshot":ref("observe")}),step("pure",pure_kind,["query"],{"entities":ref("query")}),step("prepare","prepare_program",["pure"],{"program":ref("pure")}),step("preview","preview_program",["prepare"],{"program":ref("prepare")}),step("review","wait_user_input",["preview"],{"preview":ref("preview")}),step("commit","request_commit",["review"],{"program":ref("prepare")}),step("job","wait_job",["commit"],{"intent":ref("commit")}),step("validate","validate_receipt",["job"],{"job":ref("job")}),step("finish","finish",["validate"])]
        else: steps=[step("pure",pure_kind,[],{"width":run_input("width"),"height":run_input("height")}),step("prepare","prepare_program",["pure"],{"program":ref("pure")}),step("preview","preview_program",["prepare"],{"program":ref("prepare")}),step("commit","request_commit",["preview"],{"preview":ref("preview")}),step("job","wait_job",["commit"],{"intent":ref("commit")}),step("validate","validate_receipt",["job"],{"job":ref("job")}),step("finish","finish",["validate"])]
        ...
        if skill_id.startswith("mechanical.auto"): fields.update({"entity_ids":{"type":"array","items":{"type":"string"},"minItems":1},"profile":{"const":"mechanical_mm"},"offset":{"type":"number","minimum":0.001}})
